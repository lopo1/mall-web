            ...
            <div class="tab-form">
                <form id="mobile_register_form" autocomplete="off">
                    ...
                    <div class="form-group marb8">
                        <label>手&nbsp;机&nbsp;号</label>
                        <input id="jsRegMobile" name="account" v-model="state.userInfo.mobile" type="text" placeholder="请输入您的手机号码">
                    </div>
                    <p class="error-text marb8" v-show="state.error.mobile">{{state.error.mobile}}</p>

                    <div class="clearfix">
                        <div class="form-group marb8 verify-code">
                            <label>短信验证码</label>
                            <input id="jsPhoneRegCaptcha" v-model="state.userInfo.code" type="text" placeholder="输入手机验证码">
                        </div>
                        <input class="verify-code-btn sendcode" type="button" id="jsSendCode" @click="sendMessage" :value="state.userInfo.getMessageText">
                    </div>
                     <p class="error-text marb8" v-show="state.error.code">{{state.error.code}}</p>

                    <div class="form-group marb8">
                        <label>密&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;码</label>
                        <input id="jsPhoneRegPwd" name="password_m" type="password" v-model="state.userInfo.password" placeholder="请输入6-20位非中文字符密码">
                    </div>
                     <p class="error-text marb8" v-show="state.error.password">{{state.error.password}}</p>
...
const isRegister = ()=>{
          ...
          .catch(function (error) {
            state.error.mobile = error.error.mobile?error.error.mobile:'';
            state.error.password = error.error.password?error.error.password:'';
            state.error.code = error.error.code?error.error.code:'';
          });
        }
const sendMessage = ()=>{
    var phone = state.userInfo.mobile;
    if(!(/^1(3|4|5|6|7|8|9)\d{9}$/.test(phone))){
        state.error.mobile = "手机号码不合法"
        return false;
    }
    //开启倒计时
    var countdown=60;
    ...
    function settime() {
        if (countdown === 0) {
            state.userInfo.getMessageText="免费获取验证码";
            countdown = 60;
            return;
        } else {
            state.userInfo.getMessageText="重新发送(" + countdown + ")";
            countdown--;
        }
        setTimeout(function() {
            settime()

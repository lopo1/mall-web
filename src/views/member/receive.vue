            ...
            <div class="box">
                <div class="box_1">
                    <div class="userCenterBox boxCenterList clearfix" style="_height:1%;">
                        ...
                        <div class="blank"></div>
                        <div width="100%"  v-for="(item, index) in state.receiveInfoArr" :key="index" :class="{'table-top':index>0}">
                            <div>
                                <div class="table-box">
                                ...
                                <div class="table-box">
                                    ...
                                    <div align="left" class="table-input"  bgcolor="#ffffff"><input name="consignee" type="text" class="inputBg" id="consignee_0"  v-model="item.signer_name">
                                ...
                                <div class="table-box">
                                    ...
                                    <div colspan="3" class="table-input"  align="left" bgcolor="#ffffff">
                                        <div class="addr">
                                            ...
                                            <v-distpicker :province="state.newInfo.province" :city="state.newInfo.city" :area="state.newInfo.area" @province="getProvince" @city="getCity" @area="getArea"></v-distpicker>
                                        </div>
                                    </div>
                                </div>
                                <div class="table-box">
                                    <div align="right" class="table-label" bgcolor="#ffffff">收货人姓名：</div>
                                    <div align="left" class="table-input"  bgcolor="#ffffff"><input name="consignee" type="text" class="inputBg" id="consignee_0"  v-model="newInfo.signer_name">
...
const  updateProvince =  (data:any)=> {
    let local = state.receiveInfoArr[state.currentIndex] as localType
                local.province = data.value;
}
const    updateCity = (data:any)=> {
    let local = state.receiveInfoArr[state.currentIndex] as localType
    local.city = data.value;
}
const updateArea = (data:any) =>{
    let local = state.receiveInfoArr[state.currentIndex] as localType
    local.area = data.value;
}


const getProvince = (data:any)=> {
    state.newInfo.province = data.value;
}
const getCity = (data:any)=>  {
    state.newInfo.city = data.value;
}
const getArea = (data:any)=> {
    state.newInfo.area = data.value;
}
const getReceiveInfo = () => { //获取收件人信息
    getAddress().then((response)=> {
        console.log(response.data);
        state.receiveInfoArr = response.data.data;
...
const addReceive =  ()=> { //提交收获信息
    addAddress(state.newInfo).then((response)=> {
        alert('添加成功');
        // 重置新的
        getReceiveInfo();
        state.newInfo = Object.assign({}, state.newInfoEmpty);

    }).catch(function (error) {
        console.log(error);
    });
}
const confirmUpdate =  (id:number, index:number) =>{ // 更新收获信息
    updateAddress(id, state.receiveInfoArr[index]).then((response)=> {
        alert('修改成功');
        getReceiveInfo();
    }).catch(function (error) {
        console.log(error);
...
const deleteInfo = (id:number, index:number) =>{ // 删除收获人信息

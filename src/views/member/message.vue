                    ...
                    <form action="" method="post" enctype="multipart/form-data" name="formMsg">
                        <table width="100%" border="0" cellpadding="3">
                            <tbody><tr>
                                ...
                                <td>
                                    <input type="radio" id="one" value="1" v-model="state.message_type">
                                    <label for="one">留言</label>
                                    <input type="radio" id="two" value="2" v-model="state.message_type">
                                    <label for="two">投诉</label>
                                    <input type="radio" id="three" value="3" v-model="state.message_type">
                                    <label for="three">询问</label>
                                    <input type="radio" id="four" value="4" v-model="state.message_type">
                                    <label for="four">售后</label>
                                    <input type="radio" id="five" value="5" v-model="state.message_type">
                            ...
                            <tr>
                                ...
                                <td><input name="msg_title" type="text" size="30" class="inputBg input-box" v-model="state.subject"></td>
                            </tr>
                            <tr>
                                <td align="right" valign="top">留言内容：</td>
                                <td><textarea name="msg_content" cols="50" rows="4" wrap="virtual" class="B_blue" v-model="state.message"></textarea></td>
                            </tr>
                            <tr>
...
const preview = (e:any)=> {
    state.file = e.target.files[0]; //获取文件资源
    console.log(state.file);
    let _self = this
    return new Promise((resolve, reject) => {
    console.log('文件名',state.file)
    let body = send_request()
        ...
        uploadFile(obj.host,form, state.file.normalize.toString())
        resolve(true)
    }).catch(err => {
        console.log(err)
        // reject(false)
...
const uploadFile = (url:string,param:any, file_name:string)=> {
    upload(url,param).then(res=> {
        state.imgurl=state.host+'/'+state.dir+file_name
        console.log(res)
        // alert(this.imgurl)
    })
}
const submitMessage = ()=> { //提交留言
    addMessage({
        ...
        "type":parseInt(state.message_type)
    }).then((response)=> {
        getMessage();
        state.message_type= ''
        state.subject=''
...
const getMessage = () => { //获取留言
    getMessages({}).then((response)=> {
        console.log(response.data);
        state.messageAll = response.data.data;
    }).catch(function (error) {
        console.log(error);
    });
}
const deleteMessage = (index:number, id:number) => { // 删除留言
    delMessages(id).then((response)=> {
        alert("删除成功")
        state.messageAll.splice(index,1);
    }).catch(function (error) {
        console.log(error);
    });
}

<template>
  <div id="header" class="new_header">
    <div class="hd_bar">
        <div class="bd_bar_bd cle">
            <ul class="welcome">
                ...
                <router-link :to="'/home/member/userinfo'">{{userInfo.name}}</router-link>
                &nbsp;[
            ...
            <ul id="userinfo-bar">
                <li class="more-menu" @mouseover="isShowVip=true" @mouseout="isShowVip=false"> <a>会员中心</a><i class="iconfont arrow"> </i>
                    <div class="more-bd" :class="{show:isShowVip}">
                        <div class="list">
                        <router-link :to="'/home/member/order'">我的订单</router-link>
                        <router-link :to="'/home/member/collection'">我的收藏</router-link>
                        <router-link :to="'/home/member/receive'">修改收货地址</router-link>
                        </div>
                    </div>
                </li>
                </ul>
    ...
    <div class="hd_main cle">
        <div class="logo">
        <router-link to="/home/index" class="lizi_logo">
            <img src="../../static/images/head/logo.gif" alt="慕学生鲜商城">
        </router-link>

        </div>
        <div class="search_box">
                <input class="sea_input" type="text" name="keywords" id="keyword" v-model="state.searchWord">
                <button  class="sea_submit" @click="searchSubmit">搜索</button>
        </div>
        <div class="head_search_hot">
         <span>热搜榜：</span>
          <router-link v-for="item in hotSearch" :to="'/home/search/'+item.keywords" :key="item.keywords">
            {{item.keywords}}
          </router-link>
        </div>
        <div class="intro">
        ...
        <div class="hd_nav_bd cle">
            <div class="main_nav main_nav_hover" id="main_nav">
                <div class="main_nav_link" @mouseover="overAllmenu" @mouseout="outAllmenu">
                    ...
                    <div class="main_cata" id="J_mainCata" v-show="state.showAllmenu">
                        <ul>
                            <li class="first" v-for="(item,index) in allMenuLabel" @mouseover="overChildrenmenu(index)" @mouseout="outChildrenmenu(index)">
                              <h3 style="background:url(../../static/images/head/1449088788518670880.png) 20px center no-repeat;">
                                <router-link :to="'/home/list/'+item.id">{{item.name}}</router-link> </h3>
                                <div class="J_subCata" id="J_subCata" v-show="state.showChildrenMenu ===index"  style=" left: 215px; top: 0px;">
                                    <div class="J_subView" >
                                      <div v-for="list in item.sub_category">
                                        <dl>
                                          <dt>
                                            <router-link :to="'/home/list/'+list.id">{{list.name}}</router-link>
                                          </dt>

                                          <dd>
                                            <router-link  v-for="childrenList in list.sub_category" :key="childrenList.id" :to="'/home/list/'+childrenList.id">{{childrenList.name}}</router-link>
                                          </dd>
                                        </dl>
                                        <div class="clear"></div>
                                      </div>
                ...
                <template v-for="(item,index) in allMenuLabel">
                  <li>
                    <div v-if="item.is_tab" >
                      <router-link :to="'/home/list/'+item.id" >{{item.name}}</router-link>
                    </div>
                  </li>
                </template>
            ...
            <div class="hd_cart" id="ECS_CARTINFO"  @mouseover="overShopCar" @mouseout="outShopCar">
             <router-link class="tit" :to="'/shoppingcart/cart'" target = _blank>
                    ...
                    <em class="num" id="hd_cartnum" style="visibility: visible;">{{goods_list.length}}</em></router-link>
                        <div class="list" v-show="state.showShopCar">
                            <div class="data">
                               <dl v-for="(item,index) in goods_list">
                                <dt><router-link :to="'/home/productDetail/'+item.goods_id" target = _blank><img :src="item.good_image"></router-link></dt>
                                <dd>
                                  <h4><router-link :to="'/home/productDetail/'+item.goods_id" target = _blank>{{item.good_name}}</router-link></h4>
                            ...
                            <div class="count">共<span class="red" id="hd_cart_count">1111</span>件商品哦~
                                  <!-- <p>总价:<span class="red"><em id="hd_cart_total">{{goods_list.totalPrice}}</em></span> -->
                                  <p>总价:<span class="red"><em id="hd_cart_total">1000</em></span>
                                  <router-link class="btn" :to="'/shoppingcart/cart'" target = _blank>去结算
                                  </router-link>
                                  </p>
                            </div>
                        </div>
...
let allMenuLabel = computed(() => {
  return store.getters['allMenuLabel']
})
let hotSearch = computed(() => {
  return store.getters['hotSearch']
})
let isShowVip = computed(() => {
  return store.getters['isShowVip']
})
...
const overAllmenu = ()=>{
    state.showAllmenu = true;
}
const outAllmenu = ()=>{
        state.showAllmenu = false;
}
const searchSubmit = ()=>{
    console.log('搜索内容',state.searchWord)
    if(state.searchWord){
        //跳转到登录
        router.push({ name: 'search', params: { keyword: state.searchWord }});
    }
}
const deleteGoods = (index:number,id:number) => { //移除购物车
...
const overChildrenmenu = (index:number)=>{
    state.showChildrenMenu = index;
}
const outChildrenmenu = (index:any)=>{
    state.showChildrenMenu = -1;
}
const overShopCar = ()=>{
    state.showShopCar = true;
}
const outShopCar = ()=>{
    state.showShopCar = false;
}
const getMenu = ()=>{//获取菜单
    getCategory({
    params:{}
    }).then((response)=> {
            // state.allMenuLabel = response.data
            store.commit('setAllMenuLabel', response.data);
        })
        .catch(function (error) {
            console.log(error);
        });
}
const gethotSearch = ()=>{//获取热搜
    getHotSearch({})
        .then((response)=> {
            // state.setHotSearch = response.data
            store.commit('setHotSearch', response.data);
        })
        .catch(function (error) {
            console.log(error);
        });

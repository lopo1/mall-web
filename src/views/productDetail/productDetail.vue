...
import { getGoodsDetail, getFav, addFav, delFav, addShopCart,getShopCarts,getCategory } from '@/api/api';
import {GoodInfo,CurrentInfo} from '@/store/type/detail'
import { ref,reactive,toRefs } from 'vue';
import {useRoute} from "vue-router";
 import {useStore} from "@/store/index";
const store = useStore();
const route = useRoute()
const currentInfo = reactive<CurrentInfo>({
    name: '',
    parent: '',
    category: {
        name: '',
...
desc_images: [],
    ...
    // const s = toRefs(proDetail);
const curShow = ref("");
let buyNum = ref(0);
let productId = 0;
const hasFav = ref(false);
const curIndex = ref(0);
const getDetails = ()=> { //  请求商品详情
    // console.log("请求商品详情");
    // console.log(route.params.productId);
    // console.log(parseInt(route.params.productId.toString()));
    productId = Number(route.params.productId)
    getGoodsDetail(productId,{})
    .then((response)=> {
    proDetail.value = response.data;
    ...
    getCategory({id:response.data.category.id}).then((res)=> {
        ...
        currentInfo.name = proDetail.value.name;
        currentInfo.category.name = res.data.name;
        store.commit('setCurrentInfo', currentInfo);
    //     this.$set(this.proDetail,'category',res.data.name)
    // proDetail.value.parent_category = res.data.name
    }).catch(function (error) {
    console.log(error);
    });
    curShow.value = proDetail.value.images[0];
    }).catch(function (error) {
    console.log(error);
    })
};
const reduceNum = ()=>{
    buyNum.value = buyNum.value === 1 ? 1 : buyNum.value - 1;
}
const addNum = ()=>{
...
const deleteCollect = ()=> {
    //删除收藏
    delFav(productId,{}).then((response)=> {
    console.log(response.data);
    hasFav.value = false
    }).catch(function (error) {
    console.log(error);
...
const ImageNext = ()=> {
    curIndex.value = curIndex.value === proDetail.value.desc_images.length - 1 ? proDetail.value.desc_images.length - 1 : curIndex.value + 1;
}
const replaceShow = (index:number, item:string)=> {
